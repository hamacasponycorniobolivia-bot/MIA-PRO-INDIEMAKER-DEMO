...
export default function AdminLedger() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`${API}/api/admin/ledger`, { headers: { 'Authorization': `Bearer ${token}` } })
      .then(res => res.ok ? res.json() : Promise.reject())
      .then(d => { if(Array.isArray(d)) { setData(d); setStatus('LIVE'); } })
      .catch(() => setStatus('UNAVAILABLE'));
  }, [token]);
  return (
    <div style={{ padding: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.5rem', color: 'var(--text-primary)', margin: 0 }}>{isEs ? 'Ledger Contable' : 'Accounting Ledger'}</h1>
      ...
      <div style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '1rem' }}>
        <p style={{ color: 'var(--text-primary)' }}>{data.length} {isEs ? 'registros encontrados' : 'records found'}.</p>
      </div>}
    </div>
  );
}
