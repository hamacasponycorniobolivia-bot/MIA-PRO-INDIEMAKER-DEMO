...
export default function AdminAudit() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`${API}/api/admin/audit`, { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  return (
    <div style={{ padding: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.5rem', color: 'var(--text-primary)', margin: 0 }}>{isEs ? 'Auditoría' : 'Audit Logs'}</h1>
      ...
      <div style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '1rem' }}>
        <p style={{ color: 'var(--text-primary)' }}>{data.length} {isEs ? 'registros de auditoría' : 'audit records'}.</p>
      </div>}
    </div>
  );
}
